...
CONSTRAINT fk_owns_mutualfund FOREIGN KEY(symbol)
	REFERENCES MUTUALFUND(symbol)
	DEFERRABLE INITIALLY IMMEDIATE,
CONSTRAINT ic_owns_shares CHECK
	(shares >= 0)
);
...
CREATE OR REPLACE PROCEDURE allocateDeposit(login_name VARCHAR2, totalDeposit FLOAT, transactionDate DATE)
	IS
...
BEGIN
	...
	allocateDeposit(:NEW.login, toAllocate,:NEW.t_date);
	
END;
/

--CUSTOMER
--Cannot spend more than balance by customer integrity constraint
...
--SELLS 
--cannot sell more shares than they own limited by owns integrity constraint
