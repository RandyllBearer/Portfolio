...
public class FriendsCoupon {
	...
	private static HashMap<Integer, Integer> userCoupon;
	...
	*/
	...
	/**
	* Creates a hashmap of format [key = user# 0,n-1] and [value = type of distributed coupon 0,numberOfCoupons-1]
	*/
	...
	public static int[] extend(int[] toExtend){
		try{
			if(numberOfEntries == count){	// if we have as many entries as users, then we can't add any more
				...
				userCoupon.put(new Integer(numberOfEntries-1), userCoupon.get(numberOfEntries-1) + 1);	// Increment the hashmap
				return toExtend;
			}
