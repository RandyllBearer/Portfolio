		...
		public EdgeList[] createMST(){
			...
			while(pq.isEmpty() != true){
				...
				if(marked[current.getVertexA()] == true && marked[current.getVertexB()] == true    ){	//If we have visited both vertices, then skip it
					...
					mst[current.getVertexB()].add(current);	//Only worry about adding one for now, MST formatting and all	//POSSIBLE SHALLOW COPY BUT DOUBT IT
			...
			if(numberOfNodes != numberOfEdges + 1){
				System.out.println("ERROR: Graph not fully connected, displaying connected portion from source...\n");
			}
