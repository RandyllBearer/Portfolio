...
public class myLZW {
	...
	private static double bitsRead = 0;
	private static double bitsEncoded = 0;
	private static double startRatio = 0;						//What we will compare to to see if 1.1 or not.
    ...
    public static void compress() { 
        ...
        while (input.length() > 0) {		//Iterate through the file
			...
			if(mode == 'm' || mode == 'M'){
				bitsRead = bitsRead + (s.length() * 8);		// Each ascii character is 1 byte, so if s.length() is 5, then we have 5 characters which is comprised of 40 bits.
			}
            BinaryStdOut.write(st.get(s), W);      	// Print s's encoding. [ST.get would return the i in the previous for loop]
			if(mode == 'm' || mode == 'M'){		   	// Printing out s, record how many bits we are outputting.
			...
			if(t<input.length() && code >= L && W == 16 && (mode == 'm' || mode == 'M') && firstTime == true){	//if we are about to start monitoring
					startRatio = bitsRead / bitsEncoded;
					firstTime = false;
			}else if(t<input.length() && code >= L && W == 16 && (mode == 'r' || mode == 'R')){	//If we are in reset mode, then reset
    ...
    public static void expand() {
        ...
        while (true) {
			...
			}else if(i == L && W == 16 && (mode == 'm' || mode == 'M') && firstTime == true){
					startRatio = bitsRead / bitsEncoded;
					firstTime = false;
